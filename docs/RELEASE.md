...
| 参数 | 含义 |
| --- | --- |
| `tag` | 已存在的远端版本标签，格式 `vMAJOR.MINOR.PATCH`，必须包含本次发布脚本。 |
| `platform` | `windows`、`linux` 或 `all`（默认）。 |
| `draft` | 默认勾选，完成后保留草稿；取消则在所有所选平台成功后公开发布。 |

推送分支或标签不会自动触发 Release。标签创建/推送与手动运行是独立操作。
...
`Release-Actions.ps1` 的 Prepare 将输入标签解析为确定的 commit SHA，并生成平台矩阵。
...
同一标签通过 concurrency 串行执行。取消运行也可能留下草稿和部分附件。
