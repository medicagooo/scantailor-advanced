...
- Windows x64、Linux x64、macOS ARM64/x64 编译 GUI、CLI，运行 C++ 测试和 CLI/PDF 集成测试。
  Windows 额外运行工作台、恢复、完成界面和语言测试。任何测试失败会使 CI 失败。

CI 不制作发布包、不创建 Release、不上传 Actions artifacts。
macOS 额外运行 GUI 项目 roundtrip 和部署后的运行目录检查；其他平台的 roundtrip 用例可能跳过。这不是完整 GUI 自动化验收。
...
| `platform` | `windows`、`linux`、`macos-arm64`、`macos-x64` 或 `all`（默认包含四个平台）。 |
...
| macOS ARM64 | `macos-15` | `ScanTailor-<tag>-macos-arm64.zip`、`.dmg`、`SHA256SUMS-macos-arm64.txt` |
| macOS Intel x64 | `macos-15-intel` | `ScanTailor-<tag>-macos-x64.zip`、`.dmg`、`SHA256SUMS-macos-x64.txt` |
...
依赖，Windows 终端工作台不作为 Linux 功能承诺。macOS 提供 ARM64/x64 ZIP、DMG 和各自的 SHA256SUMS 文件，详见 [macOS 使用与验证说明](MACOS.md)。本流程不提供 Windows/Linux ARM、RPM、Flatpak 发行包。
...
macOS 使用 Qt 官方预编译 Qt 6.8.3 和 Apple Clang；包仅临时签名，未公证。Windows 使用同一 MSYS2 MINGW64 包源的 Qt5、GCC、Boost 和图像库，复用
...
linuxdeploy，macOS 调用 `Package-MacOS.ps1`；生成包及 SHA256 文件。标签版本写入构建副本的统一版本模板，GUI、CLI
...
若第一次选 `all` 后某平台失败，仍建议选 `all` 重试，以对全部四个平台完成统一验收。
