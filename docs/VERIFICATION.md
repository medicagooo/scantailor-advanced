...
## CLI 3.3 task lifecycle and reuse — verified 2026-09-14 (Asia/Singapore)

Implementation: `scripts/scantailor_menu/{model,persistence,ui,workbench}.py`, `scripts/process_pdf_folder.py`, `src/cli/BatchRunner.cpp`. Applied preferences survive restart, overview distinguishes render/input and output DPI, new PDF auxiliary files live under `_scantailor`, duplicate/partial tasks require appropriate confirmation, source-only quick preview filters before processing, and verified pre-output analysis checkpoints are shared across stages. Exact whole-project preview remains available. See CLI.md and MENU.md for compatibility rules and limits.

- Offline Windows build reports 3.3.0; all 5 CTest groups pass (`build-native/reuse-build.log`).
- 72 Python integration tests pass: 12 task lifecycle, 20 full CLI/PDF including actual GUI project roundtrip, 11 encoding, 9 workbench, 8 menu, 7 original CLI and 5 completion UI. Logs: `build-native/reuse-<test_name>.log`.
- The independent Bugbot review found six issues: unordered input identities, missing referenced-image hashes for projects, orphaned manual modes in portable defaults, PDF scalar DPI overriding page rules, failed batches classified as complete, and reuse ignoring external save-project targets. Each was repaired and regression-tested. Additional repairs cover file-versus-directory output suggestions, original page_reused events, corrupt completion metadata, partial native errors in PDF results, and confirmed overwrite retaining valid stage analysis.
- Portable 3.3.0 native runtime validated with Windows-only PATH, plus the lifecycle, workbench, completion, original CLI, encoding and menu suites (`reuse-portable-*.log`). Bundled Python/PyMuPDF/Pillow exercised through the actual PowerShell wrapper with PNG level 0, TIFF LZW and JPEG quality 25; original PDF SHA unchanged (`reuse-powershell.log`).
- Supplied nine-page PDF: bundled Python and CLI, 150 DPI, PNG 6, 2 workers. Quick preview rendered exactly sources 1/5/9; repeated preview rendered none and emitted task_reused; full processing reused those three renders and prepared only 2/3/4/6/7/8. All commands exit 0, final PDF is directly in output root, source hash unchanged. Evidence: `build-native/reuse-user-pdf-33/verification.json` and sample/repeat/full JSONL. The wrapper also validates every resulting PDF page's dimensions and renderability before publication. This is workflow verification, not a claim of perfect scan quality for all documents.
