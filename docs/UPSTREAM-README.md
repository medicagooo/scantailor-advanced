> Historical reference preserved from the repository README at `a085c0896220ede7763ae6f3b666b6306c54f6d7`.
> Feature history and external distribution/build claims below describe upstream context;
> they are not release guarantees for this CLI fork. For current usage, start with
> [English](../README.md) or [简体中文](../README.zh-CN.md). Relative links were adjusted after relocation.
...
| **CI smoke build** | Historical upstream behavior: this fork no longer has a separate Flatpak workflow. See [current CI and Release instructions](RELEASE.md). |
...
**Linux – GitHub Releases:** This fork uses a manual platform-selectable Release workflow. See [current release instructions](RELEASE.md) for assets, runtime requirements, and publication controls.
...
* #### ScanTailor Universal fixes & improvements
  * Improvements for the thumbnail view.<br>
...
* #### ScanTailor Advanced fixes & improvements
  * Portability.<br>
    The settings and program files are stored in the folder with the application.<br>
  ...
  * Deviation feature reworked.<br>
    1. A deviation provider implemented.<br>
       It supports caching and recalculates the values on demand. There isn't more any necessity to store deviation in page parameters and so in the project file, that approach caused some problems as the deviation is not actually a page parameter and depends on all the pages in the project.<br>
    2. Added sorting by decreasing deviation.

  * Page/content boxes and auto margins features fixes & improvements.<br>
  ...
  * Auto and original alignment modes reworked:<br>
    ...
    6. Added ability to separately control vertical and horizontal automatic alignment when auto or original alignment mode enabled.<br>

  * Changed the way of the adjustment of the despeckle strength.<br>
  ...
  * Improvements on the thumbnails view and navigation:<br>
  ...
  * Added options in the settings to manage the quality and size of thumbnails.<br>
    It's possible to switch thumbnail quality while working on a project with the changes applied immediately.
    For every quality chosen a cache is created.<br>
...
* #### Multi-threading support for batch processing
  ...
  **Warning!** More threads requires more memory to use. Exclude situations of that to be overflowed.<br>
...
* #### New zone interaction modes<br>
  * Zone creation mode:<br>
    Press **`Z`** to switch to polygonal mode.<br>
    Press **`X`** to switch to lasso (free drawing) mode.<br>
    Press **`C`** to switch to rectangular mode.<br>
    Press **`Z`** or **`X`** while creating a new zone to switch between polygonal and rectangular modes respectively.<br>
  * Zone:<br>
    **`Shift+LMB`** on a zone - drag the zone.<br>
    **`Ctrl+Shift+LMB`** on a zone - drag the zone copying.<br>
    **`Ctrl+Alt+Click`** - copy the latest created zone to the current cursor position.<br>
    **`Del`** when the cursor is over a zone - delete the zone.<br>
  * Zone vertex:<br>
    **`D`** when the cursor is over a zone vertex - delete the vertex.<br>
    Hold **`Ctrl`** when dragging a zone vertex - make the angle of the vertex right.<br>

* #### Saving zoom and focus on switching output tabs
  The save is precise and considers the images transformations.<br>
  Also added a feature of swithing the output tabs by using **`Ctrl+1..5`** keys combinations.<br>
...
* #### Collapsible filter options.
  Now group boxes containing filter options can be collapsed/expanded.<br>
  The collapse status is preserved between restarts of the application.<br>

* #### Auto adjusting content area.
  Use **`double-click`** on content to automatically adjust the content area.<br>
...
* #### Black on white detection
  This feature allows to process images with light content on dark background correctly by correcting auto algorithms.<br>
...
* #### Guides
  ...
  These are horizontal or vertical lines you can display on a page at the margins stage \(when aligning enabled\).<br>
  *Note: Guides are adaptive to the page soft margins, i.e. when the latter changed the guides on the page
  ...
  Capabilities:<br>
    * **`Right-click`** to create/remove guides from the **context menu** called.<br>
    * **`Right-click`** on a guide to delete that guide from the **context menu** called.<br>
    * **`Ctrl+Alt+LMB`** - drag the guide under the cursor.<br>
    * **`Shift/Ctrl+LMB`** on the content rectangle - drag the page content.
      ...
      Hold **`Shift+Ctrl`** for usual dragging.<br>
    * **`Double-click`** on content - automatically attach that content to the nearest guide.
      ...
      Hold **`Shift+Ctrl`** to attach that to both the nearest vertical and horizontal guides.<br>
    * Use the **context menu** to enable/disable showing the hard margins rectangle.<br>
